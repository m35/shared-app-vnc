 ...
 *
 * 2. Whenever read() would block, it invokes the GTK event dispatching
 *    mechanism to process GTK events.
 */

#define WAIT_TIMEOUT_SEC  0
#define WAIT_TIMEOUT_USEC 100000        /* 100ms */
...
AcceptTcpConnection(int listenSock)
{
  ...
  sock = accept(listenSock, (struct sockaddr *) &addr, (SOCKLEN_T *) &addrlen);
  if (sock < 0) {
    fprintf(stderr,programName);
...
SameMachine(int sock)
{
  ...
  getpeername(sock, (struct sockaddr *)&peeraddr, (SOCKLEN_T *) &addrlen);
  getsockname(sock, (struct sockaddr *)&myaddr, (SOCKLEN_T *) &addrlen);
