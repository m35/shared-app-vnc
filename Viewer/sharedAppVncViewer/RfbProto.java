//
//  Copyright (C) 2005 Grant Wallace. gwallace@cs.princeton.edu   All Rights Reserved.
...
class RfbProto {
  ...
  final static int FramebufferUpdate = 0, SetColourMapEntries = 1, Bell = 2,
    ServerCutText = 3, SharedAppUpdate = 48, SharedAppDisconnect = 49;
  ...
  final static int
    ...
    EncodingSharedApp      = 0xFFFFFF30;

  final static int MaxNormalEncoding = 9;
  ...
  RfbProto(Socket s, Options o) throws IOException {
    sock = s;
  ...
  void setEncodings()
  {
    try {
      if (inNormalProtocol) {
        writeSetEncodings(options.getEncodings(), options.getNEncodings());
      }
    } catch (Exception e) {
      e.printStackTrace();
    }
  ...
  public void setPixelFormat() throws IOException {
    if (options.getEightBitColors()) {
      writeSetPixelFormat(8, 8, false, true, 7, 7, 3, 0, 3, 6);
      bytesPixel = 1;
    } else {
      writeSetPixelFormat(32, 24, false, true, 255, 255, 255, 16, 8, 0);
      bytesPixel = 4;
    }
  }

  public void setMultiCursor() throws IOException {
    if (options.getMultiCursor() > 0) 
      writeMultiCursor(options.getMultiCursor());
  }
  ...
  void writeClientInit() throws IOException {
    if (options.getShareDesktop()) {
      os.write(1);
    } else {
      os.write(0);
    }
    options.setShareDesktop(false);
  }
  ...
  //
  // Read a SharedAppUpdate message
  //
  ...
  boolean readSharedAppUpdate() throws IOException {
    ...
    windowId = is.readInt();
    parentId = is.readInt();
    updateRect.x = is.readUnsignedShort();
    updateRect.y = is.readUnsignedShort();
    updateRect.width = is.readUnsignedShort();
    updateRect.height = is.readUnsignedShort();
    cursorOffset.x =  is.readUnsignedShort();
    cursorOffset.y =  is.readUnsignedShort();

    //System.out.println("SharedAppUpdate 0x" + ", 0x" + Integer.toHexString(windowId) + 
    //", 0x" + Integer.toHexString(parentId) + ", " + updateRect.x + ", " + updateRect.y +
    //", " + updateRect.width + ", " + updateRect.height + ", " + updateNRects + ")");
  ...
  void writeDisconnectEvent()
       throws IOException
  {
    int b = SharedAppDisconnect;
    os.write(b);
  }
  ...
  void writePointerEvent(int winId, MouseEvent evt) throws IOException {
    int modifiers = evt.getModifiers();

    int mask2 = 2;
    int mask3 = 4;
    if (options.getReverseMouseButtons2And3()) {
      mask2 = 4;
      mask3 = 2;
    }
    ...
    eventBufLen = 0;
    writeModifierKeyEvents(winId, modifiers);
    ...
    if (bServerSupportsSharedApp)
    {
      ...
      eventBuf[eventBufLen++] = (byte) (0xad); //pad
      eventBuf[eventBufLen++] = (byte) ((winId >> 24) & 0xff);
      eventBuf[eventBufLen++] = (byte) ((winId >> 16) & 0xff);
...
//+ Integer.toHexString((winId >> 16) & 0xff) +  " "
//+ Integer.toHexString((winId >> 8) & 0xff) +  " "
//+ Integer.toHexString(winId  & 0xff)); 
    ...
    if (pointerMask == 0) {
      writeModifierKeyEvents(winId, 0);
    }
  ...
  void writeKeyEvent(int winId, KeyEvent evt) throws IOException {
    ...
    if ((key == 0xe5) || (key == 0xc5) || // XK_aring / XK_Aring
      ...
      if (!down && !brokenKeyPressed) {
        ...
        // a press. Fake it. 
        eventBufLen = 0;
        writeModifierKeyEvents(winId, evt.getModifiers());
        writeKeyEvent(winId, key, true);
        os.write(eventBuf, 0, eventBufLen);
      }
  ...
  void writeModifierKeyEvents(int winId, int newModifiers) {
    if ((newModifiers & CTRL_MASK) != (oldModifiers & CTRL_MASK))
      writeKeyEvent(winId, 0xffe3, (newModifiers & CTRL_MASK) != 0);

    if ((newModifiers & SHIFT_MASK) != (oldModifiers & SHIFT_MASK))
      writeKeyEvent(winId, 0xffe1, (newModifiers & SHIFT_MASK) != 0);

    if ((newModifiers & META_MASK) != (oldModifiers & META_MASK))
      writeKeyEvent(winId, 0xffe7, (newModifiers & META_MASK) != 0);

    if ((newModifiers & ALT_MASK) != (oldModifiers & ALT_MASK))
      writeKeyEvent(winId, 0xffe9, (newModifiers & ALT_MASK) != 0);
